var PongGame = (function(){
  ...
  var checkOverlap = function(t1, t2){
  ...
  var clamp = function(min, n, max){ return Math.min(Math.max(n, min), max); }

  var triggerKeysDown = (function(){
    var keys = { }
    var defs = { left: 37, up: 38, right: 39, down: 40, w: 87, d: 68, a: 65, s: 83 }
    var isDown = function(num){ return keys[num] || false; }
    ...
    return function(game){
      Object.keys(defs).forEach(function(k){
        if(isDown(defs[k])){ game._trigger('keydown', k) }
      })
    }
  })();

  var _playerMoveUp = function(ydist){
    var new_top = 0;
  ...
  var _playerMoveDown = function(ydist){
    _playerMoveUp.call(this, -ydist);
  }

  var _ballSetDirection = function(xdir){
    xdir = xdir < 0 ? -1 : 1;
  ...
  function PongGame(ball, field, playerLeft, playerRight){
    g_game = this;
    this.ball = g_ball = ball;
    this.field = g_field = field;
    this.playerLeft = g_playerLeft = playerLeft;
    this.playerRight = g_playerRight = playerRight;
  ...
  PongGame.prototype._updateBall = function(){
    if(this.ball){
      ...
      this.ball.style.left = this._ball_x;
      this.ball.style.top = this._ball_y;
    }
  }
  ...
  PongGame.prototype._trigger = function(ev_name){
    ...
    if(this._listeners[ev_name]){
      this._listeners[ev_name].forEach(function(fn){ fn.apply(null, args) });
    }
  }
